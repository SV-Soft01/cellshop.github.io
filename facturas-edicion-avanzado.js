// Sistema avanzado para edición de facturas sin modificar script.js
;(() => {
    console.log("Inicializando sistema avanzado de edición de facturas...")
      ...
      if (tipoRegistroSelect) {
        ...
        tipoRegistroSelect.onchange = function (event) {
          ...
          if (this.value === "facturas") {
            setTimeout(agregarBotonesEdicion, 500)
          }
        }
    ...
    // Modificar la función editarFactura para usar la búsqueda flexible
    function editarFactura(idFactura) {
      ...
      try {
        ...
        if (!factura) {
          ...
          // Mostrar un mensaje más detallado para ayudar a diagnosticar
          alert(`No se encontró la factura especificada (ID: ${idFactura}).\n\nVerifique la consola para más detalles.`)
          return
        }
    ...
    function cargarProductosFactura(productos) {
      ...
      productos.forEach((producto, index) => {
        ...
        tr.innerHTML = `
          ...
          <td>
            <input type="number" class="cantidad-producto" value="${producto.cantidad}" min="1" 
                   onchange="actualizarCantidadProducto(this, ${index})" style="width: 60px;">
          </td>
          <td>
            <input type="number" class="precio-producto" value="${producto.precio.toFixed(2)}" step="0.01" 
    ...
    // Esta función se ejecutará si el método principal falla
    ...
    console.log("Sistema de edición de facturas avanzado inicializado")
  })()
